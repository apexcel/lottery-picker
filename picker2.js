...
function getRand() {
    return parseInt(((new Date().getMilliseconds() * Math.random()) % 9), 10);
}

function annuityLotteryPicker() {
    let picked = [];
    for (let i = 0; i < 6; i++) {
        let newRand = getRand();
        picked.push(newRand);
...
function execution(howmany) {
    for (let i = 0; i < howmany; i++) {
        const pickedArray = annuityLotteryPicker()
